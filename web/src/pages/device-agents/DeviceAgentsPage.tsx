  ...
  return (
    <section>
      <div className='dashboard-header'>
        <div>
          <Typography.Title level={1}>设备管理</Typography.Title>
          <Typography.Paragraph type='secondary'>
            管理 Agent 设备、查看模型状态，设备需加入分组才能被调用。
          </Typography.Paragraph>
        </div>
        <Space>
          <Button loading={dashboard.loading && Boolean(dashboard.data)} onClick={() => void dashboard.reload()}>刷新</Button>
          <Button type='primary' onClick={() => setDeviceModalOpen(true)}>添加设备</Button>
        </Space>
      </div>
          ...
          <Row gutter={[16, 16]}>
            {agents.map((agent) => (
              <AgentCard
                key={agent.id}
                agent={agent}
                onRotate={() => void agentApi.rotate(agent.id).then(() => message.success('凭证已轮换')).catch(() => message.error(REQUEST_ERROR_MESSAGE))}
                onRevoke={() => Modal.confirm({
                  title: '撤销设备凭证？',
