...
export function DeviceGroupsPage() {
  ...
  const dashboard = useAsyncList(
    async () => {
      ...
      const details = await Promise.all(
        groups.map(
          async (group) => [group.id, await groupApi.get(group.id)] as const,
        ),
      );
      return {
        groups,
        agents,
        details: Object.fromEntries(details) as Record<string, GroupDetail>,
      };
    },
    { poll: true },
  );
  ...
  const columns: ColumnsType<GroupRow> = [
    ...
    },
    {
      ...
      width: 250,
      render: (_value, group) =>
        group.detail?.agents.length ? (
          <Space wrap size={[6, 6]}>
            {group.detail.agents.map((agent) => (
              <Tag key={agent.id} color='cyan'>
                {agent.name || agent.id}
              </Tag>
            ))}
          </Space>
    ...
    },
    {
      ...
      render: (_value, group) => {
        const models = Array.from(
          new Map(
            (group.detail?.agents ?? [])
              .flatMap((agent) => agent.modelInstances ?? [])
        ...
        return models.length ? (
          <Space wrap size={[6, 6]}>
            {models.map((model) => (
              <Tag key={model.name} color={modelColor(model.engine)}>
                {model.name}
              </Tag>
            ))}
          </Space>
  ...
  return (
    <section className='device-groups-page'>
      <PageHeader
        title='模型分组'
        subtitle='按设备组织可调用模型，集中管理关联关系。'
      />
      {dashboard.error && (
        <ErrorState onRetry={() => void dashboard.reload()} />
      )}
      <div className='group-table-toolbar'>
        <Typography.Text strong>分组列表</Typography.Text>
        <Space wrap>
          <Input
            allowClear
      ...
      <div className='group-table-shell'>
        {!rows.length && !dashboard.loading && !dashboard.error ? (
          <EmptyState
            description={
              search ? '没有匹配的分组' : '暂无分组。创建分组后即可关联设备。'
            }
          />
        ) : (
          <Table<GroupRow>
