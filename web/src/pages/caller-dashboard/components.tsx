...
export function UsageMetrics({ totalCalls, totalTokens, inputTokens, outputTokens, errorRate, avgLatencyMs }: { totalCalls: number; totalTokens: number; inputTokens: number; outputTokens: number; errorRate: number; avgLatencyMs: number }) {
  return <div className='metric-grid'><Card><Statistic title='总调用数' value={totalCalls} /></Card><Card><Statistic title='总 Token' value={totalTokens} /></Card><Card><Statistic title='输入 / 输出 Token' value={`${inputTokens} / ${outputTokens}`} /></Card><Card><Statistic title='错误率' value={(errorRate * 100).toFixed(1)} suffix='%' /></Card><Card><Statistic title='平均延迟' value={avgLatencyMs} suffix='ms' /></Card></div>;
}
...
export function ApiKeyTable({ keys, usage, hasError, onDisable, onRemove }: { keys: ApiKeySummary[]; usage: Map<string | null, { totalCalls: number; totalTokens: number }>; hasError: boolean; onDisable: (id: string) => void; onRemove: (id: string) => void }) {
  return <Card title='API Key'>{!keys.length && !hasError ? <EmptyState description='暂无 API Key' /> : <Table rowKey='id' dataSource={keys} pagination={false} scroll={{ x: 900 }} columns={[{ title: '前缀', dataIndex: 'prefix' }, { title: '渠道', dataIndex: 'channelName', render: (value: string) => value || '—' }, { title: '近 30 天 Token', key: 'totalTokens', render: (_value: unknown, record: ApiKeySummary) => Number(usage.get(record.id)?.totalTokens ?? 0).toLocaleString() }, { title: '调用次数', key: 'totalCalls', render: (_value: unknown, record: ApiKeySummary) => Number(usage.get(record.id)?.totalCalls ?? 0).toLocaleString() }, { title: '状态', dataIndex: 'status', render: (status: string) => <Tag color={status === 'active' ? 'green' : 'default'}>{status === 'active' ? '启用' : '已禁用'}</Tag> }, { title: '创建时间', dataIndex: 'createdAt', render: (value: string) => formatDate(value) }, { title: '过期时间', dataIndex: 'expiresAt', render: (value: string | null) => formatDateOnly(value) }, { title: '操作', key: 'actions', render: (_: unknown, record: ApiKeySummary) => <Space><Button disabled={record.status !== 'active'} onClick={() => onDisable(record.id)}>禁用</Button><Popconfirm title='确认删除此 API Key？' okText='删除' cancelText='取消' onConfirm={() => onRemove(record.id)}><Button danger>删除</Button></Popconfirm></Space> }]} />}</Card>;
}
