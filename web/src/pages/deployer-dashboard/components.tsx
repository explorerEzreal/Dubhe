...
export function GroupCard({ group, onEdit, onDelete, onInvite }: { group: GroupSummary; onEdit: () => void; onDelete: () => void; onInvite: () => void }) {
  return <Col xs={24} md={12} lg={8}>
    <Card
      title={group.name}
      extra={<Tag>{group.agentCount} 台设备</Tag>}
      actions={[
        <Button type='link' key='edit' onClick={onEdit}>编辑</Button>,
        <Button type='link' key='invite' onClick={onInvite}>邀请</Button>,
        <Button type='link' key='delete' danger onClick={onDelete}>删除</Button>,
      ]}
    >
      <Typography.Paragraph type='secondary'>{group.description || '暂无描述'}</Typography.Paragraph>
    </Card>
  </Col>;
}

export function CreateEditGroupModal({ open, editing, loading, onClose, onSubmit }: { open: boolean; editing: GroupSummary | null; loading: boolean; onClose: () => void; onSubmit: (values: { name: string; description?: string | null }) => void }) {
  return <Modal title={editing ? '编辑分组' : '创建分组'} open={open} onCancel={onClose} footer={null} destroyOnClose>
    <Form layout='vertical' onFinish={onSubmit} initialValues={editing ? { name: editing.name, description: editing.description ?? '' } : undefined}>
      ...
      <Button type='primary' htmlType='submit' loading={loading} block>{editing ? '保存' : '创建'}</Button>
    </Form>
  </Modal>;
}

export function InviteTokenModal({ open, token, onClose, onCopy }: { open: boolean; token: string; onClose: () => void; onCopy: () => void }) {
  return <Modal title='邀请码' open={open} onCancel={onClose} footer={<Button type='primary' onClick={onClose}>完成</Button>}>
    <Space direction='vertical' size='middle' style={{ width: '100%' }}>
      <Alert type='info' showIcon message='将以下邀请码发送给调用者，调用者可通过"添加渠道"功能接入此分组。邀请码 7 天内有效。' />
      <Typography.Text strong>邀请码（JWT）</Typography.Text>
      <Typography.Text className='code-block' copyable={{ text: token }}>{token}</Typography.Text>
      <Button onClick={onCopy}>复制邀请码</Button>
    </Space>
  </Modal>;
}

export function AddAgentToGroupModal({ open, groups, agents, loading, onClose, onSubmit }: { open: boolean; groups: GroupSummary[]; agents: AgentSummary[]; loading: boolean; onClose: () => void; onSubmit: (groupId: string, agentId: string) => void }) {
  const [selectedGroup, setSelectedGroup] = useState<string>('');
  const [selectedAgent, setSelectedAgent] = useState<string>('');
  return <Modal title='将设备加入分组' open={open} onCancel={onClose} footer={null} destroyOnClose>
    <Form layout='vertical' onFinish={() => { if (selectedGroup && selectedAgent) onSubmit(selectedGroup, selectedAgent); }}>
      <Form.Item label='选择分组' required>
        <Select placeholder='选择分组' value={selectedGroup || undefined} onChange={setSelectedGroup} options={groups.map((g) => ({ label: g.name, value: g.id }))} />
      </Form.Item>
      <Form.Item label='选择设备' required>
        <Select placeholder='选择设备' value={selectedAgent || undefined} onChange={setSelectedAgent} options={agents.map((a) => ({ label: a.name || a.id, value: a.id }))} />
      </Form.Item>
      <Button type='primary' htmlType='submit' loading={loading} block disabled={!selectedGroup || !selectedAgent}>添加</Button>
    </Form>
  </Modal>;
}
