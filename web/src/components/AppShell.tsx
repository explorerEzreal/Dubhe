...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={`app-shell ${collapsed ? 'is-collapsed' : ''}`}>
    <aside className="sidebar">
      <div className="sidebar-header"><div className="brand-mark">S</div><div className="brand-copy"><strong>ShibaWork</strong><small>v0.1.0</small></div><Tooltip title={collapsed ? '展开侧边栏' : '收起侧边栏'} placement="right"><button className="collapse-button" onClick={toggleCollapsed} aria-label={collapsed ? '展开侧边栏' : '收起侧边栏'}><Icon name="menu" /></button></Tooltip></div>
      ...
      <div className="sidebar-footer"><div className="footer-actions"><Tooltip title="切换主题" placement="right"><button className="theme-button" onClick={toggleTheme} aria-label="切换主题"><Icon name={mode === 'light' ? 'moon' : 'sun'} /></button></Tooltip></div><div className="user-profile"><span className="avatar">{(email?.[0] ?? 'U').toUpperCase()}</span><span className="user-email">{email ?? '已登录用户'}</span><Tooltip title="退出登录" placement="right"><button className="settings-button" onClick={logout} aria-label="退出登录"><Icon name="settings" /></button></Tooltip></div></div>
