...
export class PgCatalogRepository implements CatalogRepository {
  ...
  async getMonitoring(userId: string, from: Date, to: Date, monitoringScope: 'caller' | 'deployer'): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      const [overview, todayOverview, trend, groups, users, keys, models, agents, requests] = await Promise.all([
        ...
        this.pool.query(`select date_trunc('day', ir.started_at) as date, count(*)::int as "totalCalls",
          coalesce(sum(coalesce(ir.total_tokens, ir.input_tokens + ir.output_tokens, 0)),0)::float as "totalTokens"
        ...
        this.pool.query(`select ir.model_id as id, coalesce(m.name,'未知模型') as name, count(*)::int as "totalCalls",
          coalesce(sum(coalesce(ir.total_tokens, ir.input_tokens + ir.output_tokens, 0)),0)::float as "totalTokens"
          from inference_requests ir left join models m on m.id=ir.model_id where ${scope} and ir.started_at >= $2 and ir.started_at < $3
          group by ir.model_id,m.name order by "totalTokens" desc`, params),
        this.pool.query(`select ir.agent_id as id, coalesce(a.name,a.device_id,'未知设备') as name, count(*)::int as "totalCalls",
          coalesce(sum(coalesce(ir.total_tokens, ir.input_tokens + ir.output_tokens, 0)),0)::float as "totalTokens"
          from inference_requests ir left join agents a on a.id=ir.agent_id where ${scope} and ir.started_at >= $2 and ir.started_at < $3
          group by ir.agent_id,a.name,a.device_id order by "totalTokens" desc`, params),
        this.pool.query(`select ir.request_id as "requestId", ir.started_at as "startedAt", u.email, coalesce(g.name,'未分组') as "groupName",
          coalesce(ak.prefix,'已删除 Key') as "apiKeyPrefix", coalesce(m.name,'未知模型') as "modelName", coalesce(a.name,a.device_id,'未知设备') as "agentName",
